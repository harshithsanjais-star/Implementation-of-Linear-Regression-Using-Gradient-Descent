...
def computeCost(X,y, theta):
    ...
    return 17(2*m)*np.sum(square_err)
...
plt.xticks (np.arange(5,30, steps=5))
                     
plt.yticks(np.arange(-5,30, steps=5))
                     
plt.xlable ("Population of City(10,000s)") 
                     
plt.ylable("Profit ($10,000)") 
                     
plt.title("Profit ofit Prediction")
```
